...
export default function PluspetrolWellFactoryCommand() {
  ...
  const microCards = [
    { label: 'selected focus', value: selectedTruckObject?.id ?? selectedMaterialObject?.name ?? selectedAsset?.label ?? activeStageDef.name },
    { label: 'dominant constraint', value: metrics.dominantConstraint },
    { label: 'netback pulse', value: `${metrics.marginImpact >= 0 ? '+' : ''}${metrics.marginImpact.toFixed(2)} $/bbl` },
  ];
  ...
  const bottomStrip = (
    <div className="grid h-full gap-3 xl:grid-cols-[minmax(0,1fr)_300px]">
      <div className="min-w-0 overflow-hidden rounded-[1.2rem] border border-slate-200 bg-white p-3">
        <StageSpine stageStates={stageStates} activeStage={activeStage} onStageClick={handleStageClick} />
      </div>
      <div className="grid gap-2">
        <ContextBadge title="active stage" value={activeStageDef.name} />
        <div className="grid grid-cols-2 gap-2">
          <DrawerMetric label="workspace" value={activeNav.label} tone="blue" />
          <DrawerMetric label="view" value={selectedViewMode} tone="cyan" />
          <DrawerMetric label="analytics" value={STAGES.find((s) => s.id === analyticsStage)?.short ?? analyticsStage} tone="slate" />
          <DrawerMetric label="confidence" value={pct(metrics.scheduleConfidence, 0)} tone="emerald" />
        </div>
      </div>
    </div>
  );
    ...
    <div className="relative h-[100vh] min-h-[740px] w-full overflow-hidden bg-white text-[13px] text-slate-900">
      <div className="flex h-full w-full flex-col" style={{ transform: `scale(${APP_SCALE})`, transformOrigin: 'top left', width: `calc(100% / ${APP_SCALE})`, height: `calc(100% / ${APP_SCALE})` }}>
        <div className="grid h-full min-h-0 grid-cols-[144px_minmax(0,1fr)_430px] grid-rows-[68px_86px_minmax(0,1fr)_154px] bg-[linear-gradient(180deg,#ffffff,#f8fbfd_58%,#f5f7fa)]">
          <aside className="row-span-4 flex h-full flex-col border-r border-slate-200 bg-[#F7F9FB] px-3 py-4">
            ...
            <nav className="mt-2 grid gap-2">
              {navItems.map((item) => {
                ...
                return (
                  <button key={item.id} type="button" onClick={() => setActiveWorkspace(item.id)} className={`flex items-center gap-2 rounded-2xl border px-3 py-3 text-left transition ${active ? 'border-[#001C2E] bg-[#001C2E] text-white shadow-md' : 'border-slate-200 bg-white text-slate-600 hover:border-cyan-200 hover:bg-cyan-50 hover:text-[#001C2E]'}`}>
            ...
            <div className="mt-5 text-[9px] font-black uppercase tracking-[0.18em] text-slate-400">rail panels</div>
            <div className="mt-2 grid gap-2">
              {contextButtons.map((item) => {
                const Icon = item.icon;
                const active = item.id === activeContextPanel;
                return (
                  <button key={item.id} type="button" onClick={() => setActiveContextPanel(item.id)} className={`flex items-center gap-2 rounded-2xl border px-3 py-2.5 text-left transition ${active ? 'border-cyan-600 bg-cyan-50 text-cyan-700' : 'border-slate-200 bg-white text-slate-500 hover:border-cyan-200 hover:bg-cyan-50'}`}>
                    <Icon className="h-4 w-4 shrink-0" />
                    <span className="text-[10px] font-black uppercase tracking-[0.12em]">{item.label}</span>
                  </button>
                );
            ...
            <div className="mt-auto rounded-[1.25rem] border border-slate-200 bg-white p-3 text-center">
              <div className="mx-auto mb-2 h-2 w-2 rounded-full bg-emerald-500 shadow-[0_0_0_5px_rgba(16,185,129,0.12)]" />
              <div className="text-[9px] font-black uppercase tracking-[0.18em] text-slate-400">live monitoring</div>
              <div className="mt-1 text-sm font-black text-[#001C2E]">tick {simulationTick % 100}</div>
            </div>
          </aside>
          ...
          <header className="col-span-2 flex items-center justify-between gap-4 border-b border-slate-200 px-4 py-3">
            <div className="min-w-0">
              <div className="flex flex-wrap items-center gap-2">
                ...
                <span className="rounded-full border border-slate-200 bg-white px-3 py-1 text-[10px] font-black uppercase tracking-[0.18em] text-slate-500">{activeWorkspaceMeta.title}</span>
              </div>
              <div className="mt-1 text-[24px] font-black tracking-tight text-[#001C2E]">Pluspetrol Well Factory Command</div>
              <div className="text-[10px] font-semibold text-slate-500">{activeWorkspaceMeta.subtitle}</div>
            </div>
          ...
          <main className="min-h-0 border-r border-slate-200 px-4 py-4">
            <div className="grid h-full min-h-0 grid-rows-[34px_minmax(0,1fr)] gap-3">
              <div className="flex items-center justify-between gap-3 rounded-2xl bg-slate-50 px-4 py-2">
                <div className="flex items-center gap-2 text-sm font-black text-[#001C2E]">
                  <span>{activeStageDef.name}</span>
                  <span className="rounded-full bg-white px-3 py-1 text-[11px] text-slate-500">{selectedViewMode}</span>
                  <span className="rounded-full bg-cyan-50 px-3 py-1 text-[11px] text-cyan-700">analytics: {STAGES.find((s) => s.id === analyticsStage)?.short ?? analyticsStage}</span>
                </div>
                <div className="flex items-center gap-2 text-[10px] font-black">
                  <span className="rounded-full bg-white px-3 py-1 text-slate-500">{selectedAsset?.label ?? 'field overview'}</span>
                  <span className="rounded-full bg-cyan-50 px-3 py-1 text-cyan-700">{metrics.trucksActive} active trucks</span>
                  <span className="rounded-full bg-amber-50 px-3 py-1 text-amber-700">{metrics.wellsDelayed} delayed wells</span>
                </div>
              </div>
          ...
          <aside className="min-h-0 px-4 py-4">
            <div className="grid h-full min-h-0 grid-rows-[36px_minmax(0,1fr)] gap-3">
              <div className="flex items-center justify-between rounded-2xl bg-slate-50 px-4 py-2">
                <div>
                  <div className="text-[10px] font-black uppercase tracking-[0.18em] text-slate-400">context rail</div>
                  <div className="text-sm font-black text-[#001C2E]">{contextButtons.find((item) => item.id === activeContextPanel)?.label}</div>
                </div>
                <div className="text-[11px] font-semibold text-slate-500">{selectedSummaryTitle}</div>
              </div>
