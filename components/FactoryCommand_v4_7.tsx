...
type EventItem = {
  ...
  detail: string;
  minutesAgo: number;
  targetType: 'pad' | 'well' | 'rig' | 'truck' | 'material' | 'battery' | 'plant' | 'tank';
  targetId: string;
};
...
const STAGES: StageDefinition[] = [
  ...
  },
  {
    ...
    short: 'Evacuation',
    purpose: 'Treatment capacity, compression, trunk rights, nominations, and spec readiness.',
    icon: Factory,
    primaryMaterial: 'spares',
  },
  {
    ...
    short: 'Dispatch',
    purpose: 'Tank fill, batch movement, custody transfer, evacuation slots, netback, and margin.',
    icon: Warehouse,
    primaryMaterial: 'tubing',
  },
];
...
function calculateSystemMetrics(controls: WhatIfControls, scenario: ScenarioId, tick: number): SystemMetrics {
  ...
  const fracStagesToday = Math.round(
    clamp(96 * fracFactor * (1 - sandDelay * 0.052) * (1 - congestion * 0.18) * (1 - waterShort * 0.42) * (1 - dieselShort * 0.28), 46, 132),
  );
...
function generateTrucks(pads: Pad[], controls: WhatIfControls, metrics: SystemMetrics, tick: number): TruckItem[] {
  const materials: Array<{ id: MaterialId; category: string; unit: string; baseQty: number }> = [
    ...
    { id: 'spares', category: 'Waste / Backhaul', unit: 'loads', baseQty: 2 },
  ];
  ...
  return Array.from({ length: count }, (_, index) => {
    ...
    const destinationPad = pads[(index * 7 + 5) % pads.length];
    const baseSeed = index * 97 + 11;
    const progress = (deterministicNoise(baseSeed + 1) + livePhase * (0.16 + (index % 5) * 0.015) + index * 0.004) % 1;
    const congestion = controls.truckCongestion / 100;
    const materialDelay = materialMeta.id === 'sand'
      ? controls.sandDeliveryDelay * 12
      : materialMeta.id === 'water'
        ? Math.max(0, 100 - controls.waterAvailability) * 0.34
        : materialMeta.id === 'diesel'
          ? Math.max(0, 100 - controls.dieselAvailability) * 0.38
          : 0;
    const risk = clamp(18 + congestion * 52 + materialDelay * 0.72 + deterministicNoise(baseSeed + 2) * 30, 4, 98);
    const status: TruckStatus = risk > 78 ? 'Delayed' : risk > 63 ? 'Queued' : progress < 0.12 ? 'Loading' : progress > 0.87 ? 'Unloading' : progress > 0.74 ? 'At Gate' : 'Moving';
    const orderStatus = risk > 84 ? 'SAP Blocked' : risk > 70 ? 'Pending Gate' : materialMeta.id === 'sand' && controls.sandDeliveryDelay > 2 ? 'Priority' : progress > 0.72 ? 'Validated' : 'Released';
    ...
    return {
      ...
      quantity: Math.round(materialMeta.baseQty * (0.82 + deterministicNoise(baseSeed + 3) * 0.45)),
      ...
      etaMinutes: Math.round(clamp(22 + (1 - progress) * 145 + congestion * 78 + materialDelay, 5, 360)),
      queueMinutes: Math.round(clamp(metrics.truckQueueTime * 18 + deterministicNoise(baseSeed + 4) * 36 + (status === 'Queued' ? 42 : 0), 0, 240)),
      dwellMinutes: Math.round(clamp(12 + deterministicNoise(baseSeed + 5) * 56 + congestion * 35, 4, 160)),
      status,
      orderStatus,
      delayReason: status === 'Delayed' || status === 'Queued' ? delayReasons[Math.min(delayReasons.length - 1, 1 + Math.floor(deterministicNoise(baseSeed + 6) * 6))] : 'None',
      impact:
        materialMeta.id === 'sand'
          ? 'May interrupt stage continuity if gate release misses the pumping window.'
          : materialMeta.id === 'diesel'
            ? 'Reduces uninterrupted pumping, rig support, and field fleet coverage.'
            : materialMeta.id === 'water'
              ? 'Constrains frac water buffer for the active pumping window.'
              : materialMeta.id === 'chemicals'
                ? 'Can affect frac fluid or treatment execution depending on the linked operation.'
...
function chartSpecsForStage(stage: StageId) {
  const specs: Record<StageId, Array<{ title: string; subtitle: string; unit: string; type: 'area' | 'bar' | 'line' | 'composed'; primary: string; secondary: string }>> = {
    ...
    completion: [
      { title: 'Frac Stage Cadence', subtitle: 'stage velocity', unit: 'stages', type: 'area', primary: 'Stages', secondary: 'Risk' },
      { title: 'Sand Pumped', subtitle: 'sand vs continuity risk', unit: 'kt', type: 'bar', primary: 'Sand', secondary: 'Demand' },
      { title: 'Water Pumped', subtitle: 'frac water buffer', unit: 'kbbl', type: 'line', primary: 'Water', secondary: 'Constraint' },
...
function deriveAIRecommendation(params: {
  ...
  const constraints = [
    { label: 'Frac supply continuity', value: controls.sandDeliveryDelay * 15 + controls.truckCongestion * 0.24 + metrics.fracContinuityRisk * 0.45, key: 'fracSupply' },
    { label: 'Frac continuity risk', value: metrics.fracContinuityRisk, key: 'frac' },
    { label: 'Battery surface capacity', value: metrics.batteryUtilization, key: 'battery' },
    { label: 'Evacuation / treatment capacity', value: metrics.evacuationUtilization, key: 'evacuation' },
    { label: 'Inventory coverage stress', value: metrics.inventoryRisk, key: 'inventory' },
    { label: 'Schedule handoff delay', value: metrics.scheduleDelay * 12, key: 'schedule' },
  ].sort((a, b) => b.value - a.value);
  const dominant = constraints[0];
  const localFocus = selectedTruck
    ? `${selectedTruck.id} ${selectedTruck.cargo} load`
    : selectedMaterial
      ? `${selectedMaterial.name} coverage`
      : selectedAsset
        ? `${selectedAsset.label ?? selectedAsset.id} ${selectedAsset.type}`
        : stageName;
  const selectedDriver = `Focus: ${localFocus} · Global constraint: ${dominant.label}`;

  let recommendation = 'Run the next decision from the physical handoff: align field execution, logistics priority, inventory allocation, and SAP release around the current global constraint.';
  let immediateImpact = `${stageName} is absorbing ${dominant.label.toLowerCase()} through schedule confidence, queue time, and startup conversion.`;
  let secondaryImpact = `Startup conversion is ${Math.abs(metrics.productionDelta / 1000).toFixed(1)} kbbl/d from the scaling baseline after current constraints.`;
  let downstreamImpact = `Downstream impact score is ${metrics.downstreamImpactScore.toFixed(0)}/100 across batteries, treatment/evacuation, dispatch, and margin.`;
  ...
  if (selectedTruck) {
    ...
    secondaryImpact = `${selectedTruck.cargo} is tied to ${selectedTruck.linkedWell}; a missed arrival window affects the linked pad before it affects the whole field.`;
    if (selectedTruck.material === 'sand') {
      downstreamImpact = `Late sand raises frac continuity risk toward ${pct(clamp(metrics.fracContinuityRisk + selectedTruck.risk * 0.08, 0, 100))}; the production effect appears after startup conversion, not immediately at the truck gate.`;
    } else if (selectedTruck.material === 'water') {
      downstreamImpact = `Late water reduces the active frac buffer and may force a pumping sequence change before stages are actually lost.`;
    } else if (selectedTruck.material === 'diesel') {
      downstreamImpact = `Late diesel affects field fuel cover across frac pumps, rig support, construction equipment, and fleet availability.`;
    } else {
      downstreamImpact = `The downstream effect is concentrated in ${MATERIAL_META[selectedTruck.material].name} coverage and linked pad schedule confidence.`;
    }
    recommendation = selectedTruck.risk > 72
      ? `Put ${selectedTruck.id} through priority gate release, verify SAP and scale-ticket validation, and nominate one backup load from the same material family to ${selectedTruck.destination}.`
      : `Keep ${selectedTruck.id} on its planned lane and pre-clear unloading documentation to reduce dwell without disrupting higher-risk convoys.`;
    priority = 'truck';
  } else if (selectedMaterial) {
    immediateImpact = `${selectedMaterial.name} has ${selectedMaterial.coverageDays.toFixed(1)} coverage days and a projected stockout window of ${selectedMaterial.nextStockoutHours.toFixed(0)} hours.`;
    ...
    if (selectedMaterial.id === 'sand') {
      downstreamImpact = `Sand pressure affects frac continuity first, then startup timing, then production forecast and battery load.`;
      recommendation = selectedMaterial.risk > 70
        ? `Prioritize inbound sand by pad readiness and pumping window, not by request time; release the minimum loads needed to protect continuous pumping. `
    ...
    } else if (selectedMaterial.id === 'water') {
      downstreamImpact = `Water coverage affects stage sequencing and rate stability; it should not be interpreted as a treatment bottleneck unless produced-water handling is selected.`;
      recommendation = selectedMaterial.risk > 70
        ? `Protect frac-water buffer for active spreads and route recovered water loads to the pad with the shortest pumping-window exposure.`
        : `Hold the current water buffer and keep dispatch aligned with the next active spread rather than the largest pad request.`;
    } else if (selectedMaterial.id === 'diesel') {
      downstreamImpact = `Diesel is a field-fuel driver across pad construction, rigs, frac pumps, and logistics fleet; it should not be treated as a pure completion material.`;
      recommendation = selectedMaterial.risk > 70
        ? `Split fuel priority between active frac pumping, rig support, and critical fleet movements; do not consume the buffer on noncritical civil work.`
        : `Maintain fuel cover by separating rig-support and frac-pumping allocations in the dispatch plan.`;
    } else if (selectedMaterial.id === 'casing' || selectedMaterial.id === 'cement' || selectedMaterial.id === 'barite') {
      downstreamImpact = `${selectedMaterial.name} pressure primarily affects drilling cycle, wellbore handoff, and the size of the completion queue.`;
      recommendation = selectedMaterial.risk > 70
        ? `Tie ${selectedMaterial.name.toLowerCase()} release to rig sequence and casing/cementing windows, then protect only the wells that are physically ready for the next operation.`
        : `Keep ${selectedMaterial.name.toLowerCase()} replenishment aligned with rig sequence and avoid creating stranded completion inventory.`;
    } else if (selectedMaterial.id === 'chemicals') {
      downstreamImpact = `Chemicals can affect frac-fluid execution, production chemistry, or treatment quality; the system should interpret them from the selected workspace and asset.`;
      recommendation = selectedMaterial.risk > 70
        ? `Separate frac chemical demand from treatment chemical demand and prioritize the batch tied to the current operating constraint.`
        : `Maintain chemical allocation by use-case so frac, production, and treatment teams do not compete against the same generic buffer.`;
    } else if (selectedMaterial.id === 'tubing') {
      downstreamImpact = `Tubing affects startup readiness and artificial-lift conversion, not drilling or evacuation directly.`;
      recommendation = selectedMaterial.risk > 70
        ? `Prioritize tubing to wells that are otherwise ready for startup and connected to batteries with available surface capacity.`
        : `Keep tubing release synchronized with startup readiness and avoid feeding wells that are surface-constrained.`;
    } else {
      downstreamImpact = `Critical spares affect rotating equipment uptime, battery reliability, compression availability, and recovery time after failures.`;
      recommendation = selectedMaterial.risk > 70
        ? `Allocate spares to the equipment class that is currently constraining conversion: battery, compressor, frac spread, or rig.`
        : `Hold spare kits against the highest-consequence failure mode rather than distributing them evenly by area.`;
    }
    priority = selectedMaterial.id;
  } else if (dominant.key === 'fracSupply' || dominant.key === 'frac') {
    ...
    secondaryImpact = `${metrics.wellsCompletion} wells are in the completion queue; ${metrics.wellsDelayed} wells are now schedule-sensitive.`;
    ...
    recommendation = `Protect the next pumping window: rank pads by readiness, pre-release gate documentation, and move only the number of sand/water loads needed to prevent an idle frac transition.`;
    ...
    secondaryImpact = `Connected wells can start, but surface capacity is the limiting conversion step.`;
    downstreamImpact = `Evacuation readiness falls as storage pressure reaches ${pct(metrics.storagePressure)}.`;
    recommendation = `If manifold connectivity allows, reroute selected wells toward lower-loaded separation capacity; otherwise defer startup or choke back noncritical wells until spare capacity recovers above the operating threshold.`;
    ...
    secondaryImpact = `Treatment capacity and trunk rights are converting upstream growth into tank pressure rather than delivered barrels.`;
    ...
    recommendation = `Prioritize spec-ready batches, rebalance plant inlet nominations, and secure dispatch windows before approving additional startup barrels into the constrained route.`;
  } else if (scenario === 'optimizationRecovery') {
    immediateImpact = `Gate automation and route discipline are holding queue time near ${metrics.truckQueueTime.toFixed(1)}h.`;
    ...
    recommendation = `Lock the recovered lane rules for 72 hours, then expand automated release validation to water and diesel fleets while monitoring battery spare capacity.`;
  }
  ...
  return {
    ...
    secondaryImpact,
    downstreamImpact,
    costImpact: `Delivered cost ${currency(metrics.deliveredCost)}/bbl; queue-related cost component ${(metrics.truckQueueTime * 0.17).toFixed(2)} $/bbl.`,
    marginImpact: `${metrics.marginImpact >= 0 ? '+' : ''}${currency(metrics.marginImpact)}/bbl netback impact versus base plan.`,
    scheduleImpact: `${metrics.scheduleDelay >= 0 ? '+' : ''}${metrics.scheduleDelay.toFixed(1)} days across critical handoffs.`,
    recommendation,
    confidence: clamp(80 + metrics.downstreamImpactScore * 0.16 - (metrics.riskLevel === 'critical' ? 2 : 0), 74, 97),
    urgency: metrics.riskLevel,
    priority,
    propagationPath: [
      { label: 'Focus', value: localFocus, risk: selectedMaterial?.risk ?? selectedTruck?.risk ?? clamp(dominant.value, 0, 100) },
      { label: 'System constraint', value: dominant.label, risk: clamp(dominant.value, 0, 100) },
      { label: 'Frac continuity risk', value: pct(metrics.fracContinuityRisk), risk: metrics.fracContinuityRisk },
      { label: 'Logistics queue', value: `${metrics.truckQueueTime.toFixed(1)}h avg truck queue`, risk: clamp(metrics.truckQueueTime * 12, 0, 100) },
      { label: 'Startup conversion', value: `${metrics.wellsDelayed} wells delayed`, risk: clamp(metrics.wellsDelayed * 0.82, 0, 100) },
      { label: 'Surface / economics', value: `${pct(Math.max(metrics.batteryUtilization, metrics.evacuationUtilization), 1)} max util · ${metrics.marginImpact >= 0 ? '+' : ''}${currency(metrics.marginImpact)}/bbl`, risk: Math.max(metrics.batteryUtilization, metrics.evacuationUtilization, clamp(55 - metrics.marginImpact * 13, 0, 100)) },
    ],
  };
}
  ...
  const base: EventItem[] = [
    {
      ...
      detail: `Civil package closed, access road released, and fuel/water support available for rig move.`,
      minutesAgo: 4 + (tick % 3),
      targetType: 'pad',
      targetId: 'PAD-12',
    },
    {
      ...
      detail: `Cycle confidence ${pct(metrics.drillingScheduleConfidence)} after casing release and mud program hold points.`,
      minutesAgo: 7 + (tick % 4),
      targetType: 'rig',
      targetId: 'R-03',
    },
    ...
    },
    {
      ...
      stage: 'completion',
      severity: 'low',
      title: 'Frac stage 37 completed on Well PAD-18-H04',
    ...
    },
    {
      ...
      detail: `Fuel coverage is now linked to pump hours, field fleet endurance, and rig support.`,
      minutesAgo: 15,
      targetType: 'material',
      targetId: 'diesel',
    },
    {
      ...
      detail: `Battery system utilization ${pct(metrics.batteryUtilization, 1)} with water handling pressure increasing.`,
      minutesAgo: 19,
      targetType: 'battery',
      targetId: 'B-07',
    },
    {
      ...
      title: 'Frac water routing recovered after route reallocation',
      detail: `Water availability restored to ${pct(controls.waterAvailability)} for the next pumping window.`,
      minutesAgo: 22,
      targetType: 'material',
      targetId: 'water',
    },
    {
      ...
      detail: `Forecast now ${compact(metrics.forecastProduction)} bbl/d with margin impact ${metrics.marginImpact >= 0 ? '+' : ''}${currency(metrics.marginImpact)}/bbl.`,
      minutesAgo: 27,
      targetType: 'well',
      targetId: 'PAD-22-H03',
    },
        ...
        return (
          <React.Fragment key={stage.id}>
            <button
              ...
              className={`group relative flex flex-1 min-w-0 flex-col items-center justify-center gap-0.5 rounded-xl border px-1.5 py-1.5 text-center transition-all ${
                active
                  ? 'border-[#001C2E] bg-[#001C2E] text-white shadow-lg shadow-slate-900/20'
                  : 'border-slate-200 bg-white hover:border-cyan-300 hover:bg-cyan-50/40'
              }`}
            >
              <div className={`flex h-6 w-6 shrink-0 items-center justify-center rounded-lg ${active ? 'bg-white/15' : `${tone.bg} border ${tone.border}`}`}>
                <Icon className={`h-3 w-3 ${active ? 'text-white' : tone.text}`} />
              </div>
              <div className={`mt-0.5 w-full truncate text-[9px] font-black leading-tight ${active ? 'text-white' : 'text-[#001C2E]'}`}>
                {stage.short}
              </div>
              <div className={`w-full truncate text-[8px] font-semibold leading-tight ${active ? 'text-white/65' : 'text-slate-500'}`}>
                {stage.liveMetric}
              </div>
              <div className="mt-1 h-1 w-full overflow-hidden rounded-full bg-black/10">
                <div
                  className="h-full rounded-full transition-all"
                  style={{ width: `${clamp(stage.progress, 4, 100)}%`, backgroundColor: active ? '#00B5E2' : accent }}
                />
              </div>
              {!active && (
                <span className={`absolute -right-1 -top-1 h-2 w-2 rounded-full border border-white ${tone.dot}`} />
              )}
            </button>
  ...
  return (
    <div className="overflow-hidden rounded-[1.75rem] border border-slate-200 bg-white/90 shadow-[0_24px_70px_-44px_rgba(15,23,42,0.55)] backdrop-blur">
      ...
      <div className="grid gap-3 p-4 md:grid-cols-2 xl:grid-cols-1">
        ...
        <SliderControl label="Water availability" value={controls.waterAvailability} min={60} max={115} step={1} suffix="%" help="Controls frac water buffer and frac stage velocity." onChange={(value) => onControlChange('waterAvailability', value)} />
...
function materialStage(material: MaterialId): StageId {
  if (material === 'sand' || material === 'water') return 'completion';
  if (material === 'diesel') return 'pad';
  if (material === 'casing' || material === 'cement' || material === 'barite') return 'drilling';
  if (material === 'tubing') return 'production';
  if (material === 'chemicals') return 'completion';
  return 'gathering';
}
...
export default function PluspetrolWellFactoryCommand() {
  ...
  const [activeContextPanel, setActiveContextPanel] = useState<'impact' | 'detail' | 'events' | 'whatIf' | 'metrics'>('impact');
  ...
  const materialContext = {
    sand: { stage: 'completion' as StageId, chartStage: 'completion' as StageId, workspace: 'inventory' as const },
    water: { stage: 'completion' as StageId, chartStage: 'completion' as StageId, workspace: 'inventory' as const },
    diesel: { stage: 'pad' as StageId, chartStage: activeStage === 'drilling' || activeStage === 'completion' ? activeStage : 'pad', workspace: 'inventory' as const },
    casing: { stage: 'drilling' as StageId, chartStage: 'drilling' as StageId, workspace: 'inventory' as const },
    cement: { stage: 'drilling' as StageId, chartStage: 'drilling' as StageId, workspace: 'inventory' as const },
    barite: { stage: 'drilling' as StageId, chartStage: 'drilling' as StageId, workspace: 'inventory' as const },
    chemicals: { stage: 'completion' as StageId, chartStage: activeStage === 'treatment' ? 'treatment' as StageId : 'completion' as StageId, workspace: 'inventory' as const },
    tubing: { stage: 'production' as StageId, chartStage: 'production' as StageId, workspace: 'inventory' as const },
    spares: { stage: 'gathering' as StageId, chartStage: activeStage === 'treatment' ? 'treatment' as StageId : 'gathering' as StageId, workspace: 'inventory' as const },
  } satisfies Record<MaterialId, { stage: StageId; chartStage: StageId; workspace: 'inventory' | 'logistics' }>;
  ...
  const workspacePanelDefaults: Record<'command' | 'logistics' | 'inventory' | 'network' | 'performance', 'impact' | 'detail' | 'events' | 'whatIf' | 'metrics'> = {
    command: 'impact',
    logistics: 'detail',
    inventory: 'detail',
    network: 'detail',
  ...
  useEffect(() => {
    setActiveContextPanel(workspacePanelDefaults[activeWorkspace]);
  }, [activeWorkspace]);
  ...
  const analyticsStage = useMemo(() => {
    if (selectedTruckObject) return materialContext[selectedTruckObject.material].chartStage;
    if (selectedMaterial) return materialContext[selectedMaterial].chartStage;
    return activeStage;
  }, [selectedTruckObject, selectedMaterial, activeStage]);

  const chartData = useMemo(() => createChartDataByStage(analyticsStage, selectedTimeRange, metrics, whatIfControls, simulationTick), [analyticsStage, selectedTimeRange, metrics, whatIfControls, simulationTick]);
  const events = useMemo(() => createEventStream(metrics, whatIfControls, simulationTick), [metrics, whatIfControls, simulationTick]);

  const aiSelectedMaterial = activeStage === 'treatment' || activeStage === 'storage' ? null : selectedMaterialObject;
  const ai = useMemo(
    () => deriveAIRecommendation({ activeStage, controls: whatIfControls, metrics, selectedAsset, selectedTruck: selectedTruckObject, selectedMaterial: aiSelectedMaterial, scenario: activeScenario }),
    [activeStage, whatIfControls, metrics, selectedAsset, selectedTruckObject, aiSelectedMaterial, activeScenario],
  );
  ...
  const handleSelectAsset = (asset: SelectedAsset) => {
    ...
    if (asset.type === 'well') {
      ...
      setActiveStage(stage);
      setSelectedMaterial(stage === 'completion' ? 'sand' : 'tubing');
      setActiveWorkspace(syncWorkspaceForStage(stage));
      setActiveContextPanel('detail');
    }
  };
  ...
  const handleSelectTruck = (truck: TruckItem) => {
    ...
    setActiveStage(materialContext[truck.material].stage);
    setSelectedViewMode('Constraint Lens');
    setActiveWorkspace('logistics');
    setActiveContextPanel('detail');
  };
  ...
  const handleSelectMaterial = (material: MaterialId, source: 'inventory' | 'logistics' | 'event' | 'generic' = 'generic') => {
    ...
    setSelectedTruck(null);
    setActiveStage(materialContext[material].stage);
    setSelectedViewMode(material === 'sand' || material === 'water' || material === 'diesel' ? 'Constraint Lens' : 'Digital Twin');
    setSelectedAsset(item?.dependentPads[0] ? { type: 'pad', id: item.dependentPads[0], label: findPad(item.dependentPads[0])?.name ?? item.dependentPads[0] } : defaultAssetForStage(materialContext[material].stage));
    setActiveWorkspace(source === 'logistics' ? 'logistics' : source === 'event' ? syncWorkspaceForStage(materialContext[material].stage) : materialContext[material].workspace);
  ...
  const handleKpiClick = (id: string) => {
    ...
    if (id === 'inventory') {
      ...
      handleSelectMaterial(highestRisk.id, 'inventory');
      return;
    }
    if (id === 'trucks' || id === 'completion') {
      setActiveStage('completion');
      setSelectedMaterial('sand');
      setSelectedViewMode('Constraint Lens');
      setActiveWorkspace('logistics');
    ...
    if (id === 'online' || id === 'current' || id === 'forecast') {
  ...
  const handleEventClick = (event: EventItem) => {
    ...
    if (event.targetType === 'material') {
      handleSelectMaterial(event.targetId as MaterialId, 'event');
      return;
    }
  ...
  const handleControlChange = (key: keyof WhatIfControls, value: number) => {
    ...
    if (key === 'sandDeliveryDelay' || key === 'waterAvailability' || key === 'dieselAvailability' || key === 'truckCongestion') {
      setActiveStage(key === 'dieselAvailability' ? 'pad' : 'completion');
      setSelectedMaterial(key === 'waterAvailability' ? 'water' : key === 'dieselAvailability' ? 'diesel' : 'sand');
      setSelectedViewMode('Constraint Lens');
      setActiveWorkspace(key === 'dieselAvailability' ? 'inventory' : 'logistics');
      setActiveContextPanel('impact');
      ...
      setActiveWorkspace('network');
      setActiveContextPanel('metrics');
    } else if (key === 'evacuationCapacity') {
      setActiveStage('treatment');
      setSelectedMaterial(null);
      setSelectedViewMode('Margin Lens');
      setActiveWorkspace('performance');
      setActiveContextPanel('impact');
    } else if (key === 'productionUpliftTarget') {
      setActiveStage('production');
      setSelectedMaterial('tubing');
      setSelectedViewMode('Margin Lens');
      setActiveWorkspace('performance');
  ...
  const navItems = [
    { id: 'command' as const, label: 'Command', icon: Cpu },
  ...
  const contextButtons = [
    { id: 'impact' as const, label: 'Impact', icon: BrainCircuit },
    { id: 'detail' as const, label: 'Detail', icon: Layers },
    { id: 'events' as const, label: 'Events', icon: Clock3 },
    { id: 'whatIf' as const, label: 'What-if', icon: SlidersHorizontal },
    { id: 'metrics' as const, label: 'Metrics', icon: Gauge },
  ];

  const workspaceMeta = {
    command: { title: 'Command center', subtitle: 'Live operational digital twin and consequence mapping.' },
    logistics: { title: 'Logistics simulation', subtitle: 'Truck movement, queueing, dispatch, and continuity risk.' },
    inventory: { title: 'Inventory cockpit', subtitle: 'Coverage, stockout risk, inbound flow, and dependent pads.' },
    network: { title: 'Field network', subtitle: 'Pads, rigs, wells, gathering, and surface capacity.' },
    performance: { title: 'Performance & economics', subtitle: 'Forecast, delivered cost, margin, and scenario outcomes.' },
  };
  ...
  const kpiRibbon = [
    ...
    { id: 'completion', label: 'Frac stages', value: metrics.fracStagesToday.toFixed(0), sub: `${metrics.wellsCompletion.toFixed(0)} wells`, risk: riskFromScore(metrics.fracContinuityRisk) },
    { id: 'trucks', label: 'Trucks', value: metrics.trucksActive.toFixed(0), sub: `${metrics.queuedTrucks.toFixed(0)} queued`, risk: riskFromScore(metrics.truckQueueTime * 12) },
    ...
    { id: 'evacuation', label: 'Evacuation', value: pct(metrics.evacuationUtilization, 0), sub: 'plant + trunk', risk: riskFromScore(metrics.evacuationUtilization) },
  ...
  const railMetrics = [
    { label: 'logistics intensity', value: pct(clamp(metrics.trucksActive / 4 + metrics.truckQueueTime * 7, 0, 100), 0), tone: metrics.truckQueueTime > 4 ? 'rose' as const : metrics.truckQueueTime > 2 ? 'amber' as const : 'cyan' as const },
    { label: 'inventory stress', value: pct(metrics.inventoryRisk, 0), tone: metrics.inventoryRisk > 75 ? 'rose' as const : metrics.inventoryRisk > 52 ? 'amber' as const : 'cyan' as const },
    { label: 'production delta', value: `${metrics.productionDelta >= 0 ? '+' : ''}${compact(metrics.productionDelta)} bbl/d`, tone: metrics.productionDelta >= 0 ? 'emerald' as const : 'rose' as const },
    { label: 'schedule delay', value: `${metrics.scheduleDelay.toFixed(1)}d`, tone: metrics.scheduleDelay > 3 ? 'amber' as const : 'slate' as const },
    { label: 'bottleneck / margin', value: `${metrics.bottleneckSeverity.toFixed(0)} · ${metrics.marginImpact >= 0 ? '+' : ''}${metrics.marginImpact.toFixed(2)}`, tone: metrics.marginImpact >= 0 ? 'emerald' as const : 'rose' as const },
    { label: 'tick', value: `${simulationTick % 100}`, tone: 'blue' as const },
  ];
  ...
  const FocusSummary = () => (
    <div className="grid grid-cols-2 gap-2">
      <ContextBadge title="selected focus" value={selectedSummaryTitle} />
      <ContextBadge title="dominant constraint" value={metrics.dominantConstraint} />
    </div>
  );
  ...
  const SelectedDetailCard = () => (
    <div className="space-y-3">
      ...
      {selectedMaterialObject ? (
        <div className="rounded-[1.2rem] border border-cyan-200 bg-cyan-50/60 p-4">
          <div className="flex items-center justify-between gap-2">
            <div>
      ...
      <div className="rounded-[1.2rem] border border-slate-200 bg-white p-3">
        <DependencyMap
          steps={selectedTruckObject
            ...
            : selectedMaterialObject
              ? [selectedMaterialObject.name, `${selectedMaterialObject.dependentPads.slice(0, 2).join(' / ')} pads`, 'logistics demand', 'execution continuity', 'production forecast', 'margin']
  ...
  const MetricsRailCard = () => (
    <div className="space-y-3">
      <FocusSummary />
      <div className="grid gap-2">{railMetrics.map((item) => <DrawerMetric key={item.label} label={item.label} value={item.value} tone={item.tone} />)}</div>
    </div>
  );
  ...
  const centerModule = (() => {
    ...
    if (activeWorkspace === 'logistics') {
      return (
        <LogisticsSimulationLayer
          ...
          metrics={metrics}
          onSelectTruck={handleSelectTruck}
          onSelectMaterial={(material) => handleSelectMaterial(material, 'logistics')}
        />
      );
    }
    if (activeWorkspace === 'inventory') {
      return <InventoryModule inventory={inventory} selectedMaterial={selectedMaterial} metrics={metrics} onSelectMaterial={(material) => handleSelectMaterial(material, 'inventory')} />;
    }
    ...
    return <AnalyticsDeck activeStage={analyticsStage} selectedTimeRange={selectedTimeRange} onTimeRangeChange={setSelectedTimeRange} chartData={chartData} />;
  })();

  const contextPanel = (() => {
    if (activeContextPanel === 'detail') return <div className="space-y-3"><FocusSummary /><SelectedDetailCard /></div>;
    if (activeContextPanel === 'events') return <div className="space-y-3"><FocusSummary /><EventStream events={visibleEvents} onEventClick={handleEventClick} /></div>;
    if (activeContextPanel === 'whatIf') return <div className="space-y-3"><FocusSummary /><WhatIfConsole controls={whatIfControls} activeScenario={activeScenario} metrics={metrics} onControlChange={handleControlChange} onReset={() => setWhatIfControls(getScenarioControls(activeScenario))} /></div>;
    if (activeContextPanel === 'metrics') return <MetricsRailCard />;
    return (
      <div className="space-y-3">
        <FocusSummary />
        <ImpactPropagationPanel ai={ai} metrics={metrics} selectedAsset={selectedAsset} selectedTruck={selectedTruckObject} selectedMaterial={aiSelectedMaterial} />
      </div>
    );
  })();
  ...
  return (
    <div className="h-[100svh] w-full overflow-hidden bg-white text-[13px] text-slate-900">
      <div className="grid h-full min-h-0 w-full grid-cols-[104px_minmax(0,1fr)_380px] grid-rows-[60px_60px_minmax(0,1fr)_104px]">

        {/* LEFT SIDEBAR */}
        <aside className="row-span-4 flex h-full min-h-0 flex-col overflow-hidden border-r border-slate-200 bg-[#FAFBFC]">
          <div className="flex h-[60px] shrink-0 items-center justify-center border-b border-slate-200">
            <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-[#001C2E] shadow-md">
              <img src={LOGO_URL} alt="Pluspetrol" className="h-6 w-6 object-contain" />
            </div>
          </div>
          <div className="min-h-0 flex-1 overflow-y-auto px-2 py-3 space-y-3">
            <div>
              <div className="mb-1 px-1 text-[7px] font-black uppercase tracking-[0.22em] text-slate-400">Workspaces</div>
                ...
                {navItems.map((item) => {
                  ...
                  return (
                    <button
                      ...
                      className={`flex w-full flex-col items-center gap-0.5 rounded-xl py-2 px-1 text-center transition-all ${
                        active ? "bg-[#001C2E] text-white shadow-sm" : "text-slate-500 hover:bg-slate-100 hover:text-[#001C2E]"
                      }`}
                    >
                      <Icon className="h-4 w-4 shrink-0" />
                      <span className="text-[8px] font-black uppercase tracking-[0.08em] leading-none">{item.label}</span>
                    </button>
                  );
                })}
              </nav>
            ...
            <div className="border-t border-slate-200 pt-3">
              <div className="mb-1 px-1 text-[7px] font-black uppercase tracking-[0.22em] text-slate-400">Context</div>
              <nav className="space-y-0.5">
                {contextButtons.map((item) => {
                  const Icon = item.icon;
                  const active = item.id === activeContextPanel;
                  return (
                    <button
                      key={item.id}
                      type="button"
                      onClick={() => setActiveContextPanel(item.id)}
                      className={`flex w-full flex-col items-center gap-0.5 rounded-xl py-2 px-1 text-center transition-all ${
                        active ? "bg-[#00B5E2] text-white shadow-sm" : "text-slate-500 hover:bg-cyan-50 hover:text-cyan-700"
                      }`}
                    >
                      <Icon className="h-4 w-4 shrink-0" />
                      <span className="text-[8px] font-black uppercase tracking-[0.08em] leading-none">{item.label}</span>
          ...
          <div className="shrink-0 border-t border-slate-200 px-2 py-2">
            <div className="flex flex-col items-center gap-1 rounded-xl border border-slate-200 bg-white py-2 px-1">
              <span className="relative flex h-2 w-2">
                <span className="absolute inline-flex h-full w-full animate-ping rounded-full bg-emerald-400 opacity-60" />
                <span className="relative inline-flex h-2 w-2 rounded-full bg-emerald-500" />
              </span>
              <span className="text-[7px] font-black uppercase tracking-[0.2em] text-slate-400">Live</span>
              <span className="text-[10px] font-black text-[#001C2E]">T{simulationTick % 100}</span>
            </div>
          </div>
        </aside>

        {/* HEADER row 1 col 2 */}
        <header className="col-start-2 flex min-w-0 items-center justify-between gap-3 overflow-hidden border-b border-slate-200 bg-white px-4">
          <div className="flex min-w-0 flex-col gap-0.5">
            <div className="flex items-center gap-2">
              <span className="shrink-0 rounded-full border border-cyan-200 bg-cyan-50 px-2.5 py-0.5 text-[8.5px] font-black uppercase tracking-[0.18em] text-cyan-700">600-Well Ops · Command</span>
              <span className="truncate text-[14px] font-black tracking-tight text-[#001C2E]">Pluspetrol Well Factory Command</span>
            </div>
            <div className="flex items-center gap-2 text-[9px] text-slate-500">
              <span className="font-semibold truncate max-w-[180px]">Focus: {selectedSummaryTitle}</span>
              <span className="text-slate-300 shrink-0">·</span>
              <span className="font-semibold shrink-0 truncate max-w-[200px]">Constraint: {metrics.dominantConstraint}</span>
              <span className="text-slate-300 shrink-0">·</span>
              <span className={`font-black shrink-0 ${metrics.marginImpact >= 0 ? "text-emerald-700" : "text-rose-700"}`}>
                Netback {metrics.marginImpact >= 0 ? "+" : ""}{metrics.marginImpact.toFixed(2)} $/bbl
              </span>
            </div>
          </div>
          <div className="flex shrink-0 items-center gap-1.5">
            <div className="flex gap-0.5 rounded-xl border border-slate-200 bg-slate-50 p-1">
              {(["Digital Twin", "Constraint Lens", "Margin Lens"] as ViewMode[]).map((mode) => (
                <button key={mode} type="button" onClick={() => setSelectedViewMode(mode)}
                  className={`rounded-lg px-2.5 py-1 text-[8.5px] font-black transition ${
                    selectedViewMode === mode ? "bg-[#001C2E] text-white" : "text-slate-500 hover:bg-white hover:text-cyan-700"
                  }`}>
                  {mode === "Digital Twin" ? "Twin" : mode === "Constraint Lens" ? "Constraint" : "Margin"}
                </button>
              ))}
            </div>
            <div className="flex gap-0.5 rounded-xl border border-slate-200 bg-slate-50 p-1">
              {(Object.keys(SCENARIOS) as ScenarioId[]).map((sc) => (
                <button key={sc} type="button" onClick={() => handleScenarioChange(sc)}
                  className={`rounded-lg px-2 py-1 text-[8px] font-black transition ${
                    activeScenario === sc ? "bg-cyan-600 text-white" : "text-slate-500 hover:bg-white hover:text-cyan-700"
                  }`}>
                  {SCENARIOS[sc].label.split(" ")[0]}
                </button>
              ))}
            </div>
          </div>
        ...
        {/* KPI RIBBON row 2 col 2 */}
        <section className="col-start-2 flex min-w-0 items-center gap-1.5 overflow-hidden border-b border-slate-200 bg-[#F7F9FB] px-3">
          {kpiRibbon.map((item) => {
            ...
            return (
              <button
                ...
                className="group flex min-w-0 flex-1 items-center gap-1.5 rounded-lg border border-slate-200 bg-white px-2 py-1 transition hover:border-cyan-300 hover:shadow-sm"
              >
                <div className={`h-7 w-1 shrink-0 rounded-full bg-gradient-to-b ${tone.fill}`} />
                <div className="min-w-0 flex-1 overflow-hidden">
                  <div className="truncate text-[7px] font-black uppercase tracking-[0.1em] text-slate-400">{item.label}</div>
                  <div className="truncate text-[12px] font-black leading-tight text-[#001C2E]">{item.value}</div>
                  <div className="truncate text-[7px] font-semibold text-slate-500">{item.sub}</div>
                </div>
              </button>
            );
          })}
        ...
        {/* CENTER MONITOR row 3 col 2 */}
        <main className="col-start-2 min-h-0 min-w-0 overflow-hidden px-3 py-2">
          <div className="grid h-full min-h-0 grid-rows-[24px_minmax(0,1fr)] gap-1.5">
            <div className="flex items-center gap-2 rounded-lg bg-slate-50 px-3 text-[8.5px]">
              <span className="font-black text-[#001C2E]">{activeStageDef.name}</span>
              <span className="rounded-full bg-white px-2 py-0.5 font-bold text-slate-500">{selectedViewMode}</span>
              <span className="rounded-full bg-cyan-50 px-2 py-0.5 font-bold text-cyan-700">
                analytics: {STAGES.find((s) => s.id === analyticsStage)?.short ?? analyticsStage}
              </span>
              <span className="ml-auto rounded-full bg-white px-2 py-0.5 font-bold text-slate-500 truncate max-w-[160px]">{selectedAsset?.label ?? "field overview"}</span>
              <span className="rounded-full bg-amber-50 px-2 py-0.5 font-bold text-amber-700 shrink-0">{metrics.wellsDelayed} delayed</span>
            </div>
            <div className="min-h-0 overflow-hidden">{centerModule}</div>
          </div>
        </main>

        {/* STAGE TIMELINE FOOTER row 4 col 2 */}
        <footer className="col-start-2 min-w-0 overflow-hidden border-t border-slate-200 bg-white px-3 py-2">
          <div className="mb-1 flex items-center justify-between">
            <span className="text-[7px] font-black uppercase tracking-[0.22em] text-slate-400">Field Lifecycle · click to pivot</span>
            <span className="text-[7px] font-bold text-slate-400">Active: {activeStageDef.name}</span>
          </div>
        ...
        {/* RIGHT CONTEXT RAIL col 3 row 1-4 */}
        <aside className="row-span-4 col-start-3 flex h-full min-h-0 flex-col overflow-hidden border-l border-slate-200 bg-white">
          <div className="shrink-0 border-b border-slate-200 bg-gradient-to-r from-white to-cyan-50/40 px-4 py-3">
            <div className="flex items-center justify-between gap-2">
              <div>
                <div className="text-[7px] font-black uppercase tracking-[0.22em] text-slate-400">Context Rail</div>
                <div className="text-[13px] font-black text-[#001C2E]">{contextButtons.find((b) => b.id === activeContextPanel)?.label}</div>
              </div>
              <div className="max-w-[160px] truncate text-right text-[9px] font-semibold text-slate-500">{selectedSummaryTitle}</div>
            </div>
          </div>
          <div className="min-h-0 flex-1 overflow-y-auto px-4 py-3">{contextPanel}</div>
        </aside>
